        ...
        <div className="lg:col-span-3 space-y-5">
          ...
          {posts.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-xl border border-slate-200 bg-white py-16 text-center dark:border-slate-800 dark:bg-[#121B2E]">
               ...
               <h3 className="text-[11px] font-black text-slate-800 dark:text-white uppercase tracking-wide">Aliran Kelas Kosong</h3>
               <p className="text-[10px] text-slate-500 mt-1 max-w-[220px]">Belum ada pengumuman atau diskusi di kelas ini.</p>
            </div>
          ) : (
            posts.map((post) => {
              const isExpanded = expandedItems[post.id] !== false // Default true

              return (
              <div key={post.id} className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-[#121B2E]/90 overflow-hidden transition-all duration-200">
                {/* Header (Clickable for Accordion) */}
                <div 
                  onClick={() => toggleExpand(post.id)}
                  className="flex items-start justify-between p-4 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"
                >
